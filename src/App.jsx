...
export default function App() {
  ...
  const [directPlayerItem, setDirectPlayerItem] = useState(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [isKids, setIsKids] = useState(() => Boolean(user?.isKids));
    ...
    return trending.filter(
      (m) =>
        ...
        (m.genre_ids?.includes(16) ||
          ...
          (m.vote_average || 0) >= 6.5),
    );
  }, [trending, isKids]);

  const filteredTrendingTV = useMemo(() => {
    if (!isKids) return trendingTV;
    return trendingTV.filter(
      (t) =>
        !t.adult &&
        (t.genre_ids?.includes(16) ||
          t.genre_ids?.includes(10751) ||
          (t.vote_average || 0) >= 6.5),
    );
  }, [trendingTV, isKids]);
  ...
  return (
    <ErrorBoundary>
      ...
      {isPlaying && (
        <button
          type="button"
          className="netflix-player-back-btn"
          onClick={() => {
            setSelected(null);
            navigate("home");
          }}
          title="Back to REDZONE"
        >
          <svg
            width="18"
            height="18"
      ...
      <div className="netflix-main-container">
      ...
      {/* REDZONE Detail Modal (More Info) */}
      {detailItem && (
        <NetflixDetailModal
          item={detailItem}
          apiKey={effectiveApiKey}
          ...
          onPlay={handlePlayItem}
          onToggleSave={toggleSave}
          isSaved={isSaved(detailItem)}
        />
      )}

      {/* REDZONE Direct Fullscreen Video Player */}
      {directPlayerItem && (
        <RedzoneVideoPlayer
          item={directPlayerItem}
          apiKey={effectiveApiKey}
        ...
        <nav className="mobile-dock-bar" aria-label="Mobile Navigation">
          ...
          >
            <div className="mobile-dock-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill={page === "home" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <button
            className={`mobile-dock-btn ${page === "tv" ? "active" : ""}`}
            onClick={() => handleSelectTab("tv")}
          >
            <div className="mobile-dock-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill={page === "tv" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.2">
                <rect x="2" y="7" width="20" height="15" rx="2" ry="2" />
                <polyline points="17 2 12 7 7 2" />
              </svg>
            </div>
            <span className="mobile-dock-label">TV Shows</span>
            {page === "tv" && <span className="mobile-dock-dot" />}
          </button>

          <button
            className={`mobile-dock-btn ${page === "movies" ? "active" : ""}`}
            onClick={() => handleSelectTab("movies")}
          >
            <div className="mobile-dock-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill={page === "movies" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.2">
            ...
            <span className="mobile-dock-label">Movies</span>
            {page === "movies" && <span className="mobile-dock-dot" />}
          </button>

          <button
            className={`mobile-dock-btn ${page === "search" ? "active" : ""}`}
          ...
          >
            ...
            {page === "search" && <span className="mobile-dock-dot" />}
          </button>

          <button
            className={`mobile-dock-btn ${page === "my-list" ? "active" : ""}`}
            onClick={() => handleSelectTab("my-list")}
          >
            <div className="mobile-dock-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill={page === "my-list" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.2">
                <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" />
              </svg>
            </div>
            <span className="mobile-dock-label">My List</span>
            {page === "my-list" && <span className="mobile-dock-dot" />}
          </button>
        </nav>
    </ErrorBoundary>
  );
